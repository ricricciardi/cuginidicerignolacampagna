...
export function ProgressChart({ runs, dist, t = (s, v) => s.replace(/\{(\w+)\}/g, (m, k) => v?.[k] ?? m) }) {
  ...
  return (
    <svg className="chart" viewBox={`0 0 ${W} ${H}`} role="img"
         aria-label={t('Tempo sui primi {dist} di ogni corsa, da {da} a {a}', { dist, da: fmtTime(Math.max(...times)), a: fmtTime(Math.min(...times)) })}>
