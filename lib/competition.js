...
export function formToQuery(fd, errors) {
  return new URLSearchParams({
    ...
    age_grading: fd.get('age_grading') === 'on' ? '1' : '0', best_segment: fd.get('best_segment') === 'on' ? '1' : '0', err: JSON.stringify(errors),
  });
}
export function queryToForm(sp) {
  return {
    ...sp, participants: (sp.participants ?? '').split(',').filter(Boolean).map(Number),
    age_grading: sp.age_grading === '1', best_segment: sp.best_segment === '1',
  };
}
...
export function validate(input) {
  ...
  const distance_m = Number(distRaw);
  if (!/^\d+$/.test(distRaw) || distance_m < DIST_MIN || distance_m > DIST_MAX || distance_m % DIST_STEP) {
  ...
  return Object.keys(errors).length
    ...
    : { value: { name, distance_m, start_date: start, end_date: end, participants,
               age_grading: input.age_grading === 'on', best_segment: input.best_segment === 'on' } };
}
