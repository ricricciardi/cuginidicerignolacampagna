...
export function statusLine(c, now = new Date()) {
  const p = phase(c, now);
  const { start, end } = compWindow(c);
  const days = (ms) => {
    const r = remaining(ms);
    if (r.d >= 1) return `${r.d} ${r.d === 1 ? 'giorno' : 'giorni'}`;
    if (r.h >= 1) return `${r.h} ${r.h === 1 ? 'ora' : 'ore'}`;
    return `${Math.max(1, r.m)} min`;
  };
  if (p === 'before') return `Parte tra ${days(start - now)}`;
  if (p === 'running') return `In corso, finisce tra ${days(end - now)}`;
  return 'Conclusa';
}
...
export function validateName(input) {
  ...
  if (!name) return { errors: { name: 'Dai un nome alla gara.' } };
  if (name.length > NAME_MAX) return { errors: { name: `Il nome può avere al massimo ${NAME_MAX} caratteri.` } };
  return { value: { name } };
}
...
export function validate(input) {
  ...
  return Object.keys(errors).length ? { errors } : { value: { name, km, start_date: start, end_date: end } };
}
