// Gare: nome, distanza in metri (a passi di 100 m), data di inizio e di fine. Date in ora italiana:
// si parte alle 00:00 del giorno di inizio, si chiude alle 24:00 del giorno di fine.
// Le gare a km totali (total_km) non hanno distanza: vince chi somma più km nel periodo.

import { fmtDist } from './format.js';

const TZ = 'Europe/Rome';
const DATE_RE = /^\d{4}-\d{2}-\d{2}$/;
export const DIST_MIN = 100;     // metri
export const DIST_MAX = 100000;  // 100 km
export const DIST_STEP = 100;
...
// Cosa conta in una gara, in breve (impostazioni, notifiche): «5 km», «km totali».
export const fmtGoal = (c, t = plain) => (c.total_km ? t('km totali') : fmtDist(c.distance_m));
...
export function formToQuery(fd, errors) {
  return new URLSearchParams({
    name: fd.get('name') ?? '', distance_m: fd.get('distance_m') ?? '', start_date: fd.get('start_date') ?? '',
    end_date: fd.get('end_date') ?? '', participants: fd.getAll('participants').join(','),
    age_grading: fd.get('age_grading') === 'on' ? '1' : '0', best_segment: fd.get('best_segment') === 'on' ? '1' : '0',
    total_km: fd.get('total_km') === 'on' ? '1' : '0', err: JSON.stringify(errors),
  });
}
export function queryToForm(sp) {
  return {
    ...sp, participants: (sp.participants ?? '').split(',').filter(Boolean).map(Number),
    age_grading: sp.age_grading === '1', best_segment: sp.best_segment === '1', total_km: sp.total_km === '1',
  };
}
...
export function validate(input) {
  ...
  const distRaw = String(input.distance_m ?? '').trim();
  ...
  // Gara a km totali: niente distanza, e niente punteggio né miglior parziale (servono una distanza).
  const total_km = input.total_km === 'on';
  const distance_m = total_km ? null : Number(distRaw);
  if (!total_km && (!/^\d+$/.test(distRaw) || distance_m < DIST_MIN || distance_m > DIST_MAX || distance_m % DIST_STEP)) {
    errors.distance_m = 'La distanza va in metri, a passi di 100: da 100 a 100.000 (es. 500, 5000, 21100).';
  }
  ...
  return Object.keys(errors).length
    ? { errors }
    : { value: { name, distance_m, start_date: start, end_date: end, participants,
               age_grading: !total_km && input.age_grading === 'on', best_segment: !total_km && input.best_segment === 'on',
               total_km } };
}
