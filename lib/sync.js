...
// (con una gara a km totali serve ogni corsa, di qualunque lunghezza).
export async function syncScope(now = new Date()) {
  const comps = (await sql`select id, distance_m, start_date, end_date, total_km from competitions`)
  ...
  return {
    ...
    minM: Math.min(...comps.map((c) => (c.total_km ? 0 : c.distance_m))),
  };
}
...
export async function syncUser(conn, scope, maxDetails = DETAIL_BATCH) {
  ...
  return { saved, pending, error, savedIds };
}
