...
import { timeAtDistance, netElevation, rank } from './efforts.js';
import { ageGradePct } from './agegrade.js';

export async function loadCompetition(id) {
  const n = Number(id);
  if (!Number.isInteger(n) || n < 1) return null;
  const [c] = await sql`select id, name, distance_m, start_date, end_date, age_grading from competitions where id = ${n}`;
  return c ?? null;
}
...
export async function loadCompetitionFor(id, userId) {
  const n = Number(id);
  if (!Number.isInteger(n) || n < 1) return null;
  const [c] = await sql`select c.id, c.name, c.distance_m, c.start_date, c.end_date, c.age_grading from competitions c
...
export async function competitionRuns(c, userId = null) {
  ...
  return rows.map((r) => {
    const time_s = timeAtDistance(r, c.distance_m);
    ...
    // Dislivello dai parziali al km: solo per distanze a km interi.
    const elev_m = c.distance_m % 1000 === 0 ? netElevation(r.splits, c.distance_m / 1000) : null;
