...
export async function loadCompetition(id) {
  ...
  const [c] = await sql`select id, name, distance_m, start_date, end_date, age_grading, best_segment from competitions where id = ${n}`;
  return c ?? null;
}
...
export async function loadCompetitionFor(id, userId) {
  const n = Number(id);
  if (!Number.isInteger(n) || n < 1) return null;
  const [c] = await sql`select c.id, c.name, c.distance_m, c.start_date, c.end_date, c.age_grading, c.best_segment from competitions c
...
export async function periodActivities(c) {
  const { start, end } = compWindow(c);
  return sql`
    select a.user_id, a.distance_m, a.moving_time_s, a.start_date, a.start_date_local
    from activities a
