...
// Tempo in parole, per le notifiche: «1 ora, 7 minuti e 3 secondi», «10 minuti e 23 secondi».
export const fmtTimeWords = (s) => {
  const h = Math.floor(s / 3600), m = Math.floor((s % 3600) / 60), sec = Math.round(s % 60);
  const parts = [
    h && `${h} ${h === 1 ? 'ora' : 'ore'}`,
    m && `${m} ${m === 1 ? 'minuto' : 'minuti'}`,
    sec && `${sec} ${sec === 1 ? 'secondo' : 'secondi'}`,
  ].filter(Boolean);
  return parts.length > 1 ? `${parts.slice(0, -1).join(', ')} e ${parts.at(-1)}` : parts[0] ?? '0 secondi';
};
