...
import { fmtTimeWords } from './format.js';
...
async function watchedCompetitions(now = new Date()) {
  const comps = await sql`select id, name, distance_m, start_date, end_date, best_segment, total_km from competitions`;
...
// Le gare a km totali non hanno tempi né record: restano fuori.
export async function standingsSnapshot() {
  const snap = {};
  for (const c of await watchedCompetitions()) {
    if (c.total_km) continue;
...
export async function notifyRaceDays(now = new Date()) {
  ...
  const comps = await sql`select id, name, distance_m, start_date, end_date, total_km from competitions
  ...
  for (const c of comps) {
    ...
    if (c.start_date === today && await once(`start:${c.id}:${today}`)) {
      sent += await sendToUsers(ids, (t) => ({ title: t('Parte oggi {gara}', { gara: c.name }),
        body: t(c.total_km ? 'Conta la somma dei km, fino al {al}. In bocca al lupo!' : 'Primi {dist}, fino al {al}. In bocca al lupo!',
                { dist: fmtDist(c.distance_m), al: c.end_date.split('-').reverse().join('/') }), url: `/gare/${c.id}`, tag: `gara-${c.id}` }));
    }
    if (c.end_date === today && await once(`end:${c.id}:${today}`)) {
      sent += await sendToUsers(ids, (t) => ({ title: t('Ultimo giorno per {gara}', { gara: c.name }),
        body: t(c.total_km ? 'Hai tempo fino a mezzanotte per aggiungere km.' : 'Hai tempo fino a mezzanotte per migliorare il tuo tempo.'), url: `/gare/${c.id}`, tag: `gara-${c.id}` }));
    }
  }
  return sent;
}
...
export async function notifyAdded(comp, userIds, byUserId) {
  ...
  return sendToUsers(ids, (t) => ({ title: t('Sei in gara: {gara}', { gara: comp.name }),
    body: comp.total_km
      ? t('L\'amministratore ti ha aggiunto a {gara}: conta la somma dei km.', { gara: comp.name })
      : t('L\'amministratore ti ha aggiunto a {gara}: primi {dist}.', { gara: comp.name, dist: fmtDist(comp.distance_m) }), url: `/gare/${comp.id}`, tag: `gara-${comp.id}` }));
}
...
// Nuovo tempo: per ogni corsa appena arrivata che vale in una gara, avviso a tutti i partecipanti
// di quella gara, compreso chi ha corso.
// «stat atteint, vè chien!!» resta così in tutte le lingue (voluto).
export async function notifyNewTimes(userId, activityIds) {
  if (!activityIds?.length) return 0;
  const ids = new Set(activityIds.map(String));
  let sent = 0;
  for (const c of await watchedCompetitions()) {
    const mine = (await competitionRuns(c, userId)).filter((r) => ids.has(String(r.id)) && r.time_s != null);
    if (!mine.length) continue;
    const first = (mine[0].athlete_name ?? '').split(/\s+/)[0] || 'Un cugino';
    const everyone = await participantIdsOf(c.id);
    for (const r of mine) {
      sent += await sendToUsers(everyone, (t) => ({
        title: t('Nuovo tempo {tempo}', { tempo: fmtTimeWords(r.time_s) }),
        body: `${first} stat atteint, vè chien!!`,
        url: `/gare/${c.id}/atleta/${userId}`, tag: `tempo-${c.id}-${r.id}`,
      }));
    }
  }
  return sent;
