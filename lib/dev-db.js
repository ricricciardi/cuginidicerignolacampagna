...
function splits(km, pace, jitter) {
  return Array.from({ length: km }, (_, i) => {
    const s = Math.round(pace + Math.sin(i * 1.7 + jitter) * 8);
    return { m: 1000, s: s + (i % 5 === 4 ? 6 : 0), mv: s };
  });
}

async function seed(db) {
  const people = [
    ['riccardo@example.com', 'Riccardo R.', 285],
    ['marco@example.com', 'Marco C.', 300],
    ['giulia@example.com', 'Giulia D.', 312],
    ['antonio@example.com', 'Antonio F.', 330],
  ];
  for (const [i, [email, name]] of people.entries()) {
    await db.query(`insert into users (email, password_hash) values ($1, 'x')`, [email]);
    await db.query(
