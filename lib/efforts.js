...
// Km più veloce di una corsa: il parziale al km (splits_metric) più breve, tra quelli completi.
// Per la classifica «Miglior parziale», solo per curiosità. null se non ci sono parziali.
export function bestKmSplit(splits) {
  const full = (Array.isArray(splits) ? splits : []).filter((x) => (x.m ?? 0) >= 1000 - TOLERANCE_PER_KM_M && x.s > 0);
  return full.length ? Math.min(...full.map((x) => x.s)) : null;
}
...
// Per le gare con il «miglior parziale» attivo. null se non si può sapere.
...
// Tempo di una corsa in una gara: miglior parziale o primi metri, secondo l'impostazione della gara.
