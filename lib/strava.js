...
// Revoca l'accesso dell'app su Strava. Se il token non vale più l'accesso è già revocato.
export async function deauthorize(accessToken) {
  const res = await fetch('https://www.strava.com/oauth/deauthorize', {
    method: 'POST',
    headers: { Authorization: `Bearer ${accessToken}` },
  });
  if (!res.ok && res.status !== 401) throw new StravaError(res.status, await res.text());
}
...
// Stream di un'attività: tempo e distanza punto per punto, per i tempi ogni 100 m (una chiamata API).
export async function fetchStreams(accessToken, id) {
  const res = await fetch(`${API}/activities/${id}/streams?keys=time,distance&key_by_type=true`, {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
  if (!res.ok) throw new StravaError(res.status, await res.text());
  const s = await res.json();
  return { time: s.time?.data ?? [], distance: s.distance?.data ?? [] };
}

// Profilo dell'atleta: nome e foto aggiornati (una chiamata API).
export async function fetchAthlete(accessToken) {
  const res = await fetch(`${API}/athlete`, { headers: { Authorization: `Bearer ${accessToken}` } });
  if (!res.ok) throw new StravaError(res.status, await res.text());
  return res.json();
}
