...
// Quadratini della striscia dell'impegno. Fino a 6 settimane uno per giorno (acceso o spento);
// oltre (es. una gara di un anno) uno per settimana, più acceso quanti più giorni ha corso.
// level: 0 niente, 1 un giorno, 2 due o tre giorni, 3 quattro o più (per giorno: 0 o 3).
export const WEEKLY_AFTER_DAYS = 42;
export function dayCells(days, ranDays) {
  if (days.length <= WEEKLY_AFTER_DAYS) {
    return days.map((d) => ({ key: d, from: d, to: d, n: ranDays.has(d) ? 1 : 0, level: ranDays.has(d) ? 3 : 0 }));
  }
  const cells = [];
  for (let i = 0; i < days.length; i += 7) {
    const week = days.slice(i, i + 7);
    const n = week.filter((d) => ranDays.has(d)).length;
    cells.push({ key: week[0], from: week[0], to: week.at(-1), n, level: n === 0 ? 0 : n === 1 ? 1 : n <= 3 ? 2 : 3 });
  }
  return cells;
}
...
export function improvement(runs, people) {
  const by = new Map(people.map((p) => [p.user_id, { ...p, first: null, best: null, count: 0, points: [] }]));
  ...
  for (const r of sorted) {
    ...
    if (!e) continue;
    e.count++;
    e.points.push(r); // per il grafico nel tempo
    e.first ??= r;
