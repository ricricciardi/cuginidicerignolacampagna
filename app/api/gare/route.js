...
export async function POST(req) {
  ...
  const [c] = await sql`insert into competitions (name, distance_m, start_date, end_date, age_grading, best_segment, total_km, created_by)
                        values (${value.name}, ${value.distance_m}, ${value.start_date}, ${value.end_date},
                                ${value.age_grading}, ${value.best_segment}, ${value.total_km}, ${userId})
                        returning id`;
